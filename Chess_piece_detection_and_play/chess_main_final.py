...
# Construct the relative path to the Stockfish binary
stockfish_path = os.path.join(script_dir, 'Engine', 'stockfish-ubuntu-x86-64-avx2')

engine = chess.engine.SimpleEngine.popen_uci(stockfish_path) # stockfish engine
...
def show_game(game_img,board,player_move):
    ...
    # Resize the final game image
    game_img = cv2.resize(game_img, (game_img.shape[1] // 2, game_img.shape[0] // 2))
...
# print(f"Top-left corner is at: ({x}, {y})")
...
# Create an initial blank image to display
blank_image = np.zeros((480, 640, 3), np.uint8)
cv2.imshow('Phone Camera', blank_image)

if not cap.isOpened():
    print("Error: Could not open video stream")
    exit()
# Function to display text on the image
def display_text(img, text, position, font=cv2.FONT_HERSHEY_SIMPLEX, font_scale=0.7, color=(255, 255, 255), thickness=1, line_height=30):
    y = position[1]
...
# Display the text
display_text(blank_image, "Do you want to set camera Position[y/n] : \nPress q for exit : ", (10, 250))
cv2.imshow('Phone Camera', blank_image)
while True:
    flag,img=cap.read()
    key =cv2.waitKey(1)
...
# Display the text
display_text(blank_image, "Do you want to warp prespective image[y/n] :", (10, 250))
cv2.imshow('Phone Camera', blank_image)
while True:
    key =cv2.waitKey(1)
    flag,img=cap.read()  
...
blank_image = np.zeros((500, 500, 3), np.uint8)
# Display the text
display_text(blank_image, "do you want to calibrate new Points for corners [y/n]:", (10, 250))
cv2.imshow('Phone Camera', blank_image)
...
blank_image = np.zeros((500, 500, 3), np.uint8)
# Display the text
display_text(blank_image, "Do you want to see Boxex on Chess board [y/n]:", (10, 250))
cv2.imshow('Phone Camera', blank_image)
...
print("Arduino all gucci? [y/n] :",end=" ")
answer = 'n'
while(answer =='n'):
    answer = str(input())
...
while 1:
    ...
    if board.turn and board.is_checkmate() == False:
        ...
        arduino.write((str(result.move) + '\n').encode())
        pass
        print("press 'w' when player moved ")#TODO wait for arduino signal
        while True:
            flag,img=cap.read()
    ...
    if board.is_checkmate():
        ...
        cv2.waitKey(0)
        break
# Close the engine and the serial connection
engine.quit()
arduino.close()
